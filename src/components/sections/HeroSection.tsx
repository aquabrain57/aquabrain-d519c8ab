  ...
  return <section id="accueil" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        ease: "easeInOut"
          ...
          <img src={slide.image} alt="AQUABRAIN" className="w-full h-full object-cover" />
      ...
      <div className="container mx-auto px-4 relative z-10 text-center">
        ...
        <div className="flex justify-center gap-2 mt-10 sm:mt-16">
          {slides.map((_, index) => <button key={index} onClick={() => goToSlide(index)} className={`h-1.5 sm:h-2 rounded-full transition-all duration-500 ${index === currentSlide ? "w-6 sm:w-10 bg-gold" : "w-1.5 sm:w-2 bg-primary-foreground/40 hover:bg-primary-foreground/60"}`} aria-label={`Aller au slide ${index + 1}`} />)}
        </div>
